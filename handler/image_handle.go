...
func imageMessageHandle(ctx *openwechat.MessageContext) {
	...
	if err != nil {
		...
		if flag {
			//fileUrl := fmt.Sprintf("https://%v/%v/%v", config.OssConfig.Endpoint, config.OssConfig.BucketName, fileName)
			fileUrl := fmt.Sprintf("/%v/%v", config.Config.OssConfig.BucketName, fileName)
